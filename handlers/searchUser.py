...
async def search(call: types.CallbackQuery):
    ...
    await bot.send_message(call.from_user.id,
                           f"<b>✏️ Напишите ID пользователя:\n\nВаш ID: <code>{call.from_user.id}</code></b>",
                           reply_markup=await default.back(), parse_mode="html")

    await states.search.user_id.set()


@dp.message_handler(state=states.search.user_id)
...
# Начать сделку
@dp.callback_query_handler(text=["deal"])
...
@dp.callback_query_handler(text=["TONdeal", "USDTdeal", "BUSDdeal", "USDCdeal"])
...
@dp.message_handler(state=states.deal.sum)
async def state(message: types.Message, state: FSMContext):
    if message.text == 'Назад ↩':
        await state.finish()
        wait = await bot.send_message(message.chat.id, '<b>Секунду...</b>', parse_mode="html",
...
# Отправить сделку
@dp.callback_query_handler(text=["sendDeal"])
